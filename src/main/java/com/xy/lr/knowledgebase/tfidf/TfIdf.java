...
//		System.out.println(spl.length);
	...
	public Map<String, Double> tfidf_tweak1() {
		...
		for (String s : documents) {
			...
			for (String d : split) {
				if (map.get(d) == null) {
//					System.out.println(tf_scores.get(d).getCounter());
//					System.out.println(df_scores.get(d).getCounter());
					double score = tf_scores.get(d).getCounter() * 
							(Math.log(N / df_scores.get(d).getCounter()) + 0.01);
					map.put(d, score);
				}
			}
		}
